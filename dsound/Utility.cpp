...
BOOL Utility::MemSearchh(BYTE* bSearchData, int nSearchSize, DWORD_PTR dwStartAddr, DWORD_PTR dwEndAddr, BOOL bIsCurrProcess, int iSearchMode, std::vector<DWORD_PTR>& vRet)
{
    ...
    while (VirtualQueryEx(Game::hProcessF, (LPCVOID)dwAddress, &mbi, sizeof(mbi)) && (dwAddress < dwEndAddr) && ((dwAddress + mbi.RegionSize) > dwAddress))
    {
    ...
    for (it = m_vMemoryRegion.begin(); it != m_vMemoryRegion.end(); it++)
    {
        ...
        else
        {
            ...
            ReadProcessMemory(Game::hProcessF, (LPCVOID)mData.dwBaseAddr, pCurrMemoryData, mData.dwMemorySize, &dwNumberOfBytesRead);
...
void  Utility::RPM(LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesRead)
{
    ReadProcessMemory(Game::hProcess, lpBaseAddress, lpBuffer, nSize, lpNumberOfBytesRead);
...
void Utility::AOBREP(BYTE BypaRep[], BYTE write[], SIZE_T size, SIZE_T sizee, int numbers)
{
    ...
    Utility::MemSearch(BypaRep, size, 0x00000000, 0x7fffffff, false, 0, Bypassdo);
